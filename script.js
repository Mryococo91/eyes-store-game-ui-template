document.addEventListener('DOMContentLoaded', function() {
  ...
  (function() {
    ...
    function ensureCopyright() {
      // Control flag to prevent duplicate execution
      if (window._ensuringCopyright) {
        return 0;
      }
      ...
      try {
        ...
        // Limit the processing to avoid infinite loops
    ...
    function monitorDOMChanges() {
      const bodyObserver = new MutationObserver(() => {
        // Prevent rapid-fire execution during page load
        if (!window._lastCopyrightCheck || Date.now() - window._lastCopyrightCheck > 1500) {
          window._lastCopyrightCheck = Date.now();
          ensureCopyright();
        }
      });
      ...
      // Reduce check interval frequency to prevent loop problems
      setInterval(() => {
        if (!window._lastCopyrightCheck || Date.now() - window._lastCopyrightCheck > 2000) {
          window._lastCopyrightCheck = Date.now();
          ensureCopyright();
        }
      }, 3000);
    ...
    function initCopyrightProtection() {
      // Use a flag to track if initialization is already in progress
      ...
      // Single timeout with proper tracking
      setTimeout(() => {
        window._lastCopyrightCheck = Date.now();
        ensureCopyright();
      }, 1000);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Re-run number animation every 10 seconds
  ...
  // Legal dropdown menüsünü görünür yapalım
  const legalDropdown = document.querySelector('.group .absolute');
  if (legalDropdown) {
    // Legal kategorisindeki sayfalarda olup olmadığımızı kontrol et
    ...
    if (isLegalPage) {
      // Legal sayfalarındayken dropdown'ı sürekli görünür yap
      ...
      // Hover olayını devre dışı bırakmak için group class'ını kaldırma
